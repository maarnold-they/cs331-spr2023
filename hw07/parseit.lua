...
local function matchString(s)
  if lexstr == s then advance() return true
...
local function matchCat(c)
  if lexcat == c then advance() return true
  else return false end
end
...
function parse_stmt_list()
  ...
  while true do
    if lexstr == "write" or lexstr == "function" or lexstr == "if" or lexstr == "while" or lexstr == "return" or lexcat == lexit.ID then
...
function parse_statement()
  ...
  if matchString("write") then
    ...
    if matchString(")") then return true, { WRITE_STMT } end
  ...
  elseif matchString("function") then
    savelex = lexstr
    if not (matchCat(lexit.ID) and matchString("(") and matchString(")")) then return false, nil end
    good, ast1 = parse_stmt_list()
    if not (good and matchString("end")) then return false, nil end
    return true, { FUNC_DEF, savelex, ast1 }
  
  elseif matchString("while") then
    good, ast1 = parse_expr()
    if not (good and matchString("do")) then return false, nil end
  ...
  elseif matchString("if") then
    ...
    if not (good and matchString("then")) then return false, nil end
    good, ast2 = parse_stmt_list()
    if not good then return false, nil end
    ...
    elseif matchCat(lexit.ID) then
      if matchString("(") then
        if matchString (")") then return true, { FUNC_CALL, savelex } 
        else return false, nil end
      ...
      elseif matchString("[") then
        good, ast1 = parse_expr()
        if not (good and matchString("]") and matchString("=")) then return false, nil end
...
function parse_write_arg()
  ...
  elseif matchString("char") then
    if not matchString("(") then return false, nil end
    good, ast = parse_expr()
    if not (good and matchString(")")) then return false, nil end
...
function parse_expr()
  ...
  while true do 
    saveop = lexstr
    if not (matchString('and') or matchString('or')) then break end
...
function parse_compare_expr()
  ...
  while true do 
    saveop = lexstr
    if not (matchString('==') or matchString('!=') or matchString('<') or matchString('<=') or matchString('>') or matchString('>=')) then break end
...
function parse_term()
  ...
  while true do
    saveop = lexstr
    if not (matchString('*') or matchString('/') or matchString('%')) then break end
...
function parse_factor()
  ...
  if matchCat(lexit.ID) then
    ...
    else return true, { SIMPLE_VAR, savelex } end
